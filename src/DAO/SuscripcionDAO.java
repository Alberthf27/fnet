...
public class SuscripcionDAO {
    ...
    public List<Suscripcion> listarTodo(String busqueda, String orden) {
        ...
        String sql = "SELECT s.id_suscripcion, s.codigo_contrato, s.direccion_instalacion, s.fecha_inicio, "
                ...
                // SINCRONIZADO CON LA REGLA DEL DIA 16/17
                // Si estamos dia 1-16: mostrar 5 meses atras + actual (offset = 0)
                // Si estamos dia 17+: mostrar 4 meses atras + actual + siguiente (offset = 1)
                + "(SELECT COALESCE("
                + "  (SELECT CASE WHEN id_estado = 2 THEN '1' ELSE '0' END FROM factura "
                + "   WHERE id_suscripcion = s.id_suscripcion "
                + "   AND LOWER(periodo_mes) LIKE CONCAT('%', LOWER(ELT(MONTH(DATE_SUB(DATE_ADD(CURDATE(), INTERVAL IF(DAY(CURDATE()) >= 17, 1, 0) MONTH), INTERVAL 5 MONTH)),'enero','febrero','marzo','abril','mayo','junio','julio','agosto','septiembre','octubre','noviembre','diciembre')), '%')"
                + "   AND LOWER(periodo_mes) LIKE CONCAT('%', YEAR(DATE_SUB(DATE_ADD(CURDATE(), INTERVAL IF(DAY(CURDATE()) >= 17, 1, 0) MONTH), INTERVAL 5 MONTH)), '%') LIMIT 1), '2')) as m1, "
                + "(SELECT COALESCE("
                + "  (SELECT CASE WHEN id_estado = 2 THEN '1' ELSE '0' END FROM factura "
                + "   WHERE id_suscripcion = s.id_suscripcion "
                + "   AND LOWER(periodo_mes) LIKE CONCAT('%', LOWER(ELT(MONTH(DATE_SUB(DATE_ADD(CURDATE(), INTERVAL IF(DAY(CURDATE()) >= 17, 1, 0) MONTH), INTERVAL 4 MONTH)),'enero','febrero','marzo','abril','mayo','junio','julio','agosto','septiembre','octubre','noviembre','diciembre')), '%')"
                + "   AND LOWER(periodo_mes) LIKE CONCAT('%', YEAR(DATE_SUB(DATE_ADD(CURDATE(), INTERVAL IF(DAY(CURDATE()) >= 17, 1, 0) MONTH), INTERVAL 4 MONTH)), '%') LIMIT 1), '2')) as m2, "
                + "(SELECT COALESCE("
                + "  (SELECT CASE WHEN id_estado = 2 THEN '1' ELSE '0' END FROM factura "
                + "   WHERE id_suscripcion = s.id_suscripcion "
                + "   AND LOWER(periodo_mes) LIKE CONCAT('%', LOWER(ELT(MONTH(DATE_SUB(DATE_ADD(CURDATE(), INTERVAL IF(DAY(CURDATE()) >= 17, 1, 0) MONTH), INTERVAL 3 MONTH)),'enero','febrero','marzo','abril','mayo','junio','julio','agosto','septiembre','octubre','noviembre','diciembre')), '%')"
                + "   AND LOWER(periodo_mes) LIKE CONCAT('%', YEAR(DATE_SUB(DATE_ADD(CURDATE(), INTERVAL IF(DAY(CURDATE()) >= 17, 1, 0) MONTH), INTERVAL 3 MONTH)), '%') LIMIT 1), '2')) as m3, "
                + "(SELECT COALESCE("
                + "  (SELECT CASE WHEN id_estado = 2 THEN '1' ELSE '0' END FROM factura "
                + "   WHERE id_suscripcion = s.id_suscripcion "
                + "   AND LOWER(periodo_mes) LIKE CONCAT('%', LOWER(ELT(MONTH(DATE_SUB(DATE_ADD(CURDATE(), INTERVAL IF(DAY(CURDATE()) >= 17, 1, 0) MONTH), INTERVAL 2 MONTH)),'enero','febrero','marzo','abril','mayo','junio','julio','agosto','septiembre','octubre','noviembre','diciembre')), '%')"
                + "   AND LOWER(periodo_mes) LIKE CONCAT('%', YEAR(DATE_SUB(DATE_ADD(CURDATE(), INTERVAL IF(DAY(CURDATE()) >= 17, 1, 0) MONTH), INTERVAL 2 MONTH)), '%') LIMIT 1), '2')) as m4, "
                + "(SELECT COALESCE("
                + "  (SELECT CASE WHEN id_estado = 2 THEN '1' ELSE '0' END FROM factura "
                + "   WHERE id_suscripcion = s.id_suscripcion "
                + "   AND LOWER(periodo_mes) LIKE CONCAT('%', LOWER(ELT(MONTH(DATE_SUB(DATE_ADD(CURDATE(), INTERVAL IF(DAY(CURDATE()) >= 17, 1, 0) MONTH), INTERVAL 1 MONTH)),'enero','febrero','marzo','abril','mayo','junio','julio','agosto','septiembre','octubre','noviembre','diciembre')), '%')"
                + "   AND LOWER(periodo_mes) LIKE CONCAT('%', YEAR(DATE_SUB(DATE_ADD(CURDATE(), INTERVAL IF(DAY(CURDATE()) >= 17, 1, 0) MONTH), INTERVAL 1 MONTH)), '%') LIMIT 1), '2')) as m5, "
                + "(SELECT COALESCE("
                + "  (SELECT CASE WHEN id_estado = 2 THEN '1' ELSE '0' END FROM factura "
                + "   WHERE id_suscripcion = s.id_suscripcion "
                + "   AND LOWER(periodo_mes) LIKE CONCAT('%', LOWER(ELT(MONTH(DATE_ADD(CURDATE(), INTERVAL IF(DAY(CURDATE()) >= 17, 1, 0) MONTH)),'enero','febrero','marzo','abril','mayo','junio','julio','agosto','septiembre','octubre','noviembre','diciembre')), '%')"
                + "   AND LOWER(periodo_mes) LIKE CONCAT('%', YEAR(DATE_ADD(CURDATE(), INTERVAL IF(DAY(CURDATE()) >= 17, 1, 0) MONTH)), '%') LIMIT 1), '2')) as m6 "
                + "FROM suscripcion s "
                + "INNER JOIN cliente c ON s.id_cliente = c.id_cliente "
                + "INNER JOIN servicio sv ON s.id_servicio = sv.id_servicio "
